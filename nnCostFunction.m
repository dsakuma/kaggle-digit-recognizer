...
z2 = a1 * Theta1'; %m x input_layer_size+1 | input_layer_size+1 x hidden_layer_size ==> m x hidden_layer_size
a2 = sigmoid(z2); %m x hidden_layer_size
a2 = [ones(m,1) a2]; %m x hidden_layer_size+1
z3 = a2 * Theta2'; %m * hidden_layer_size+1 | hidden_layer_size+1 x num_labels  ==> m x num_labels
...
Beta = -y_vec .* log(a3) - (1-y_vec) .* log(1-a3);
J = sum(sum(Beta)) / m;
...
for t=1:m
	a1 = X(t,:)';
	a1 = [1 ; a1]; %input_layer_size+1 x 1
	z2 = Theta1 * a1; %hidden_layer_size x input_layer_size+1 | input_layer_size+1 x 1 ==> hidden_layer_size x 1
